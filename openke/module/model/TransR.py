...
class TransR(Model):
	...
	def _transfer(self, e, r_transfer):  # e:[batch_e, dim_e]
		r_transfer = r_transfer.view(-1, self.dim_e, self.dim_r)  # [batch_r, dim_e, dim_r]
		if e.shape[0] != r_transfer.shape[0]:
			e = e.view(-1, r_transfer.shape[0], self.dim_e).permute(1, 0, 2)  # 维度换位 [-1, batch_r, dim_e] -> [batch_r, -1, dim_e]
			e = torch.matmul(e, r_transfer).permute(1, 0, 2)
		else:
			e = e.view(-1, 1, self.dim_e)  # [batch_e, 1, dim_e]
	...
	def forward(self, data):
		...
		mode = data['mode']
		h = self.ent_embeddings(batch_h)  # [batch_size, dim_e]
		t = self.ent_embeddings(batch_t)
		r = self.rel_embeddings(batch_r)
		r_transfer = self.transfer_matrix(batch_r)  # [batch_r, dim_e*dim_r]
