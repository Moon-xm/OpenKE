...
f.write("%d\n"%(len(rellef)))
for i in rellef:
	f.write("%s\t%d"%(i,len(rellef[i])))  # rel
	for j in rellef[i]:  # head
		f.write("\t%s"%(j))
	f.write("\n")
	f.write("%s\t%d"%(i,len(relrig[i])))  # rel
	for j in relrig[i]:  # tail
		f.write("\t%s"%(j))
	f.write("\n")
f.close()
...
for i in lef:
	if not i[1] in rellef:
		rellef[i[1]] = 0
		totlef[i[1]] = 0
	rellef[i[1]] += len(lef[i])  # {'0': 369, '1': 1426, '2': 2901,...}  每个rel对应几个head
	totlef[i[1]] += 1.0  # {'0': 192.0, '1': 271.0,...}  rel计数

for i in rig:
	if not i[0] in relrig:
		relrig[i[0]] = 0
		totrig[i[0]] = 0
	relrig[i[0]] += len(rig[i])  # 每个rel对应几个tail
	totrig[i[0]] += 1.0  # rel 计数
